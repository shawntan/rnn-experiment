...
def load_network(filename):
	...
	X,Y,tunables,predictions = construct_network(p['context'],p['characters'],p['hidden'])
	for W,pW in zip(tunables,p['tunables']): W.set_value(pW)
	return X,predictions
if __name__ == '__main__':
	w = "%s$%s\n"%(sys.argv[2],"_"*len(sys.argv[2]))
